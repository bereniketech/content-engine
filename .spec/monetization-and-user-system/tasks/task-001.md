...
# Task 001: Supabase Schema Migration — All Tables
...
## Objective
Create the complete initial Postgres schema (enums, all business tables, monthly RANGE partitions, indexes, and constraints) for the monetization and user system in a single Supabase migration.
...
| File | Purpose |
|------|---------|
| `supabase/migrations/20260428000000_init_monetization.sql` | Initial schema migration: enums, tables, partitions, indexes, constraints |
...
# (none — pure SQL migration)

# Env vars (names only — add values to .env):
SUPABASE_URL=Supabase project URL
SUPABASE_SERVICE_ROLE_KEY=Supabase service role key (server only)
SUPABASE_DB_URL=Postgres connection string used by supabase CLI
```
...
### `supabase/migrations/20260428000000_init_monetization.sql`
```sql
-- =====================================================================
...
### Key Code Snippets
_(none — first migration in repo)_

### Key Patterns in Use
- **Single-migration init:** all enums + tables emitted in one timestamped file so a fresh `supabase db reset` builds the entire schema deterministically.
- **Partition by month:** every append-heavy table (`credit_transactions`, `email_log`, `trust_score_events`, `abuse_logs`, `user_ip_log`) is RANGE partitioned on `created_at`; primary keys include `created_at` to satisfy partitioning.
- **`gen_random_uuid()`** from `pgcrypto` for all UUID PKs.

### Architecture Decisions Affecting This Task
- ADR: Atomic credit ops via SECURITY DEFINER RPCs (Task 3) — schema must support `FOR UPDATE` row locks on `credit_wallets.balance`.
- ADR: Idempotency on every credit mutation — enforced by `UNIQUE (wallet_id, request_id, created_at)` on `credit_transactions`.
- ADR: One active subscription per user — enforced via partial unique index `uniq_active_sub_per_user`.
...
1. `supabase/migrations/20260428000000_init_monetization.sql` — write full DDL above
2. Run: `supabase db reset` (local) to apply
3. Run: `supabase db diff --schema public` — confirm zero drift
4. Run: `/verify`
...
### File: `supabase/tests/schema.test.sql`
```sql
...
| Scenario | Action |
|----------|--------|
| Wallet `balance` would go below 0 | Postgres raises CHECK violation `new row for relation "credit_wallets" violates check constraint "credit_wallets_balance_check"` |
| Duplicate `(wallet_id, request_id)` insert in `credit_transactions` | Postgres raises `duplicate key value violates unique constraint "credit_transactions_wallet_id_request_id_created_at_key"` |
| Second active subscription for same user | Postgres raises `duplicate key value violates unique constraint "uniq_active_sub_per_user"` |
| `trust_score` outside 0–100 | Postgres raises CHECK violation `users_trust_score_check` |

---

## Acceptance Criteria
- [ ] WHEN `supabase db reset` runs THEN migration applies with zero errors
- [ ] WHEN `supabase test db` runs THEN all 12 schema assertions pass
- [ ] WHEN inserting two active subscriptions for the same user THEN second insert is rejected
- [ ] WHEN inserting duplicate `(wallet_id, request_id)` into `credit_transactions` THEN insert is rejected
...
**Files changed:** `supabase/migrations/20260428000000_init_monetization.sql`, `supabase/tests/schema.test.sql`
**Decisions made:** Used exact DDL from task spec; all 6 enums, 20 tables with partitioned credit_transactions/email_log/trust_score_events/abuse_logs/user_ip_log; partial unique index uniq_active_sub_per_user.
**Context for next task:** Schema defines tables in `public` schema; RLS policies must reference these table names exactly.
**Open questions:** None.
